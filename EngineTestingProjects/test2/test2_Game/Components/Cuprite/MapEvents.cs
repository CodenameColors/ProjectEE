...
namespace ProjectE_E.Components.Cuprite
{
	...
	#region level1.lvl
    public static void NoMoveHere_Dele(ref Player player)
    {
        Console.WriteLine(String.Format("Collision Activated {0}", "WIP"));
    }
    ...
    //Used for changing the map/level. given a new position
    public static void Level1_2Transfer_Dele(GraphicsDevice graphicsDevice, SpriteBatch spriteBatch, Player player ,Map CurrentMap, String FileName, int newx, int newy, int movetime)
    {
        //get the level file!
        Level NewLevel = Level.ImportLevel(FileName);
        //Create New Map
    ...
    //Used for changing the map/level. given a new position
    public static void Level2_1Transfer_Dele(GraphicsDevice graphicsDevice, SpriteBatch spriteBatch, Player player ,Map CurrentMap, String FileName, int newx, int newy, int movetime)
    {
        //get the level file!
        Level NewLevel = Level.ImportLevel(FileName);
        //Create New Map
